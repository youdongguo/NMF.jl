...
@testset "gradient in update rule" begin
    ...
    for j in 1:8
        ...
        w1 = rand(length(w_j))
        grad_w = .5 .*ForwardDiff.gradient(weightedresidual_w, w1)
        grad_w1 = ((M.^2)*(h_j.^2)).*w1 .- (M.^2 .*(X-W*H+w_j*h_j'))*h_j
    ...
    for j in 1:8
        ...
        h1 = rand(length(h_j))
        grad_h = .5 .*ForwardDiff.gradient(weightedresidual_h, h1)
        grad_h1 = ((M.^2)'*(w_j.^2)).*h1 .- (M.^2 .*(X-W*H+w_j*h_j'))'*w_j
